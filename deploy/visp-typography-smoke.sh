...
if [[ -n ${S3_BUCKET:-} ]]; then
	# The dashboard (BRB images, Studio assets) uploads straight to the bucket
	# too, so its origin needs the same CORS rule as Typography's.
	for origin in "$2" "$1"; do
		response=$(curl --fail --silent --show-error --max-time 15 --include \
			-X OPTIONS -H "Origin: https://$origin" \
			-H 'Access-Control-Request-Method: PUT' \
			-H 'Access-Control-Request-Headers: content-type' \
			"${S3_UPLOAD_ENDPOINT:-${S3_ENDPOINT:?}}/${S3_BUCKET}/typography/cors-check")
		printf '%s\n' "$response" | grep -Fiq "access-control-allow-origin: https://$origin"
		printf 'ok: storage accepts browser upload preflight from %s\n' "$origin"
	done
fi
