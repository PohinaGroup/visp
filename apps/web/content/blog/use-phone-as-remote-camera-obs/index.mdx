...
The relay does not transcode the incoming feed. Choose a resolution, frame rate,
and bitrate the phone and mobile connection can sustain. VISP Native can
duplicate packets over Wi-Fi and cellular without aggregating their capacity;
read the [mobile-network resilience
guide](/blog/keep-stream-live-bad-mobile-network) if the route has weak
coverage.

## Step 1: Sign in and choose the phone workflow

Open VISP and sign in with the Twitch, Kick, or Google identity used for the production.
...
hash; HTTPS obtains a one-use ticket and live control uses outbound WSS. OBS
does not listen on a public remote-control port.
