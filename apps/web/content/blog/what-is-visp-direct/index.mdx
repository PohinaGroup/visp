...
description: "Learn how VISP Direct sends one phone feed to Twitch, Kick, YouTube, or any combination without OBS, who benefits, and when OBS remains useful."
...
**VISP Direct** sends one publishing device to Twitch, Kick, YouTube, or any
combination without
...
for a multi-platform simulcast. It is not a better version of every VISP
...
4. OBS mixes the show, encodes the finished program, and sends it to its chosen
   destinations.
...
it using credentials or a broadcast destination VISP obtains after you
authorize that provider.
OBS is no longer required to make the public stream happen.

If you select multiple outputs, the phone still sends only one contribution.
The relay runs one independent forwarding process per selected destination.
...
| Who encodes for the destination? | The VISP relay, once per destination | OBS |
...
| Does VISP use multiple mobile connections? | The native app can duplicate packets over Wi-Fi and cellular; it does not aggregate capacity | The same native publishing option is available |
| Is Direct generally available? | Yes, for Twitch, Kick, and YouTube | OBS remains available as an optional read path |
...
- Twitch, Kick, YouTube, or a combination should receive that same program;
...
For YouTube, VISP creates or selects a public live broadcast and binds an RTMPS
stream after the Google account grants YouTube access.
...
touches destination credentials. Direct is an explicit, per-device option
...
relay forwarding slot; selecting all three providers consumes three. If the relay is full,
...
changing the selected provider checkboxes. This avoids changing
destinations under a live contribution.

**Native publishing can use two links.** The native app can duplicate packets
over Wi-Fi and cellular so either copy may arrive, but it does not aggregate
their bandwidth. If both uplinks disappear, Direct has no media to forward.
Creators who need capacity aggregation should add an appropriate network layer; the
...
**Every platform encode uses relay resources.** Selecting more destinations
increases Direct CPU work and outbound media bandwidth once per destination. It
does not increase the phone upload.
...
1. Create or sign in to VISP with Twitch, Kick, or Google.
2. Add the phone as its own publishing device.
3. Open **Direct output** in the dashboard.
4. Authorize the Twitch, Kick, and/or YouTube outputs you need.
5. While the device is offline, select the destination checkboxes.
...
that should reach Twitch, Kick, and/or YouTube without OBS. Keep OBS when the studio is part
