...
description: "Learn how VISP Direct sends one phone feed to Twitch, Kick, or both without OBS, who benefits, and where the limited beta does not fit."
...
VISP Direct is also a **limited beta**. Access is enabled per account because
each destination consumes a full encode on the current single relay node.
...
| Is Direct generally available? | No, it is a limited beta | The normal relay workflow is the standard VISP path |
...
- your account has limited-beta access; and
...
**It is a limited beta.** A Direct destination consumes one of the relay's
concurrent forwarding slots. Twitch plus Kick consumes two. If the relay is at
capacity, the Direct output is refused while the ordinary relay-to-OBS path
continues working.

**One device owns each provider.** You may send one phone to Twitch and a
different device to Kick, or one device to both. You cannot configure two of
your devices to publish to Twitch at the same time. The database constraint,
not only the interface, enforces this ownership.
...
If your account has beta access:
