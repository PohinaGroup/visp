...
export function reorderStudioLayer(
	...
	return {
		...
		scenes: graph.scenes.map((scene) => {
			...
			return {
				...scene,
				layers: layers.reverse().map((layer, zIndex) => ({ ...layer, zIndex })),
			};
		}),
	};
}
...
export function resizeStudioLayer(
	origin: LayerRect,
	handle: ResizeHandle,
	dx: number,
	dy: number,
	...
	let width =
		handle === "n" || handle === "s"
			? origin.width
			: origin.width + (west ? -dx : dx);
	let height =
		handle === "e" || handle === "w"
			? origin.height
			: origin.height + (north ? -dy : dy);
	...
	if (ratio) {
		...
		const scale = Math.max(
			1 / Math.min(origin.width, origin.height),
			Math.min(
				width / origin.width,
...
export function snapStudioPosition(rect: LayerRect, tolerance: number) {
	const snap = (position: number, size: number, frame: number) => {
		const candidates = [
			{ position: 0, guide: 0 },
			{ position: (frame - size) / 2, guide: frame / 2 },
			{ position: frame - size, guide: frame },
		];
		const closest = candidates.sort(
			(a, b) =>
				Math.abs(position - a.position) - Math.abs(position - b.position),
		)[0];
		return closest && Math.abs(position - closest.position) <= tolerance
			? closest
			: { position, guide: undefined };
	};
...
export const emptyStudioHistory: StudioHistory = {
	past: [],
	future: [],
	dirty: false,
	seq: 0,
...
export function editStudioHistory(
	state: StudioHistory,
	graph: StudioGraph,
): StudioHistory {
	...
	return {
		...
		future: [],
		past:
			state.graph && !state.gesture
				? [...state.past, state.graph].slice(-50)
				: state.past,
	};
}
export function endStudioGesture(state: StudioHistory): StudioHistory {
	return {
		...
		past:
			state.gesture &&
			JSON.stringify(state.gesture) !== JSON.stringify(state.graph)
				? [...state.past, state.gesture].slice(-50)
				: state.past,
	};
}
export function travelStudioHistory(
	state: StudioHistory,
	...
	return {
		...
		[opposite]: [...state[opposite], state.graph].slice(-50),
	};
}
export function savedStudioHistory(
	state: StudioHistory,
	graph: StudioGraph,
	seq: number,
	version: number,
	...
	return {
		...state,
		version,
		...(state.seq === seq ? { graph, dirty: false } : {}),
	};
}
