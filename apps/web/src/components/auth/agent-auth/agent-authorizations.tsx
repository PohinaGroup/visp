...
import {
	...
	AlertDialogMedia,
	AlertDialogTitle,
} from "@VISP/ui/components/alert-dialog";
...
export function AgentAuthorizations({ className }: AgentAuthorizationsProps) {
	...
	const trpc = useTRPC();
	const activity = useQuery(trpc.agent.activity.queryOptions());
	...
	return (
		<div className={cn("flex flex-col gap-3", className)}>
			...
			<Card className="p-0">
				<CardContent className="flex flex-col gap-5 p-4">
					{agents.isPending ? (
						...
						<p role="alert">{plugin.localization.approvalError}</p>
					) : agents.data?.length ? (
			...
			<div className="flex flex-col gap-1">
				...
				<p className="text-muted-foreground text-xs">
					Successful reads made by agents you approved.
				</p>
			</div>
			<Card className="p-0">
				<CardContent className="flex flex-col gap-3 p-4">
					{activity.isPending ? (
						...
						<p role="alert">Could not load agent activity.</p>
					) : activity.data?.length ? (
						activity.data.map((entry) => (
							<div key={entry.id} className="flex flex-col gap-1 text-sm">
								<span>
									{entry.agentName} read {entry.target}
								</span>
