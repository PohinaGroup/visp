...
relay path. Direct sends that feed to Twitch, Kick, or YouTube. OBS can optionally read
...
The iOS build ships through the
[App Store](https://apps.apple.com/fi/app/visp-easy-streaming/id6791948092) and
Android through Google Play. Client links live on
...
1. Sign in with Twitch, Kick, or Google. The app creates a publishing device for this
...
- **OBS chat overlay** stays blank until a chat message arrives; append `&debug=1`
  to its Browser Source URL to show connection status.
...
- **Stream info sheet** edits the title and metadata on the provider without
  leaving the app. Twitch and Kick support title/category editing; YouTube's
  default Direct title is configured separately.
- **Picture in Picture** can keep supported iPhone capture visible while you use
  another app. Background behavior still depends on the device and OS.
- **Subtitles** can burn English or Finnish speech into the camera picture.
  On-device recognition is the default; hosted recognition appears only for
  accounts where it is enabled.
- **Audio isolation** is available on supported iPhones, with an optional hosted
  mode where enabled. Test it before relying on it live.
