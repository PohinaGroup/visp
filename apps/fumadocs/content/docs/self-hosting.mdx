...
4. Build and install services:
   ...
   `APP_DOMAIN`, `ADMIN_DOMAIN`, `NATIVE_WEB_DOMAIN`, `DOCS_DOMAIN`, and
   `RELAY_PUBLIC_IP`. Caddy serves `apps/admin/dist`, `apps/native/dist`, and
...
6. Expose only public **TCP 443**. Allow SSH only over Tailscale. Create DNS
   for the app, admin, browser-publisher, and docs hostnames before Caddy
