...
detection. `/api/obs/control` must stay publicly reachable over HTTPS so the
OBS plugin can poll from the broadcaster's computer.
...
Deploy the API and Caddy rules, and apply migrations, before pairing OBS.
...
1. Open **OBS remote control** in the portal and generate a token.
2. Start OBS once so it logs the `config.ini` path.
3. Close OBS and write:
...
only invokes start and stop. Treat `config.ini` as a machine credential.
