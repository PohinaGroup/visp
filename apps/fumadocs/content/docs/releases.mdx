...
browser broadcaster, OBS Remote web app, and this documentation site; and
attaches the OBS plugin packages to the same GitHub Release. Native store
...
six app-host artifacts **before** restarting anything. It validates the Caddy
...
OBS Remote native store distribution, native store promotion, OBS plugin
installation on broadcaster machines, automatic database rollback, and relay
restarts are intentionally outside the workflow. A merge to `main` publishes
compatible VISP native OTA updates to XPRem's staging branch through
`native-update.yml`.
