...
/** A user-visible record of completed agent capability calls. */
export const agentActivity = pgTable(
	...
	(table) => [
		index("agent_activity_user_created_at_idx").on(
			table.userId,
			table.createdAt,
		),
		index("agent_activity_agent_created_at_idx").on(
			table.agentId,
			table.createdAt,
		),
	],
...
export const agentAction = pgTable(
	...
	(table) => [
		index("agent_action_user_status_idx").on(table.userId, table.status),
